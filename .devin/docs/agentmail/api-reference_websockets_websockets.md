    ...
    publish:
      ...
      message:
        oneOf:
          ...
          - $ref: >-
              #/components/messages/subpackage_websockets.websockets-server-7-message_opened
          - $ref: >-
              #/components/messages/subpackage_websockets.websockets-server-8-domain_verified
          - $ref: >-
              #/components/messages/subpackage_websockets.websockets-server-9-error
    subscribe:
...
components:
  messages:
    ...
    subpackage_websockets.websockets-server-7-message_opened:
      name: message_opened
      payload:
        $ref: '#/components/schemas/type_events:MessageOpenedEvent'
    subpackage_websockets.websockets-server-8-domain_verified:
      name: domain_verified
      payload:
        $ref: '#/components/schemas/type_events:DomainVerifiedEvent'
    subpackage_websockets.websockets-server-9-error:
      name: error
      payload:
  ...
  schemas:
    ...
    type_events:Open:
      ...
      required:
        ...
        - timestamp
      title: Open
    type_events:MessageOpenedEvent:
      type: object
      properties:
        type:
          type: string
        ...
        event_id:
          $ref: '#/components/schemas/type_events:EventId'
        open:
          $ref: '#/components/schemas/type_events:Open'
      required:
        - type
        - event_type
        - event_id
        - open
      description: >-
        A tracked message was opened for the first time. Sent once per message:
        repeat opens do not

        resend it.
      title: MessageOpenedEvent
    type_pods:PodId:
      type: string
    ...
    type_domains:TrackingEnabled:
      type: boolean
      description: >-
        Serve open tracking pixels from this domain. Adds a required
        `link.<domain>`
    ...
    type_domains:VerificationRecord:
      ...
      properties:
        ...
        reason:
          type: string
          description: >-
            Why the record is INVALID, when known. `duplicate_records` means the
            expected value is present but extra records coexist at the same
            name; `value_mismatch` means a record exists but does not match the
            expected value.
      required:
        - type
        - name
    ...
    type_domains:Domain:
      ...
      properties:
        ...
        reason:
          type: string
          description: >-
            Why the domain is not (yet) VERIFIED, when known.
            `dns_records_missing` / `dns_records_invalid` point at the DNS
            records. The `ses_*` values mean the records look right and
            sending-infrastructure validation has not converged:
            `ses_dkim_pending` / `ses_mail_from_pending` (still checking),
            `ses_dkim_temporary_failure` / `ses_mail_from_temporary_failure` (a
            transient error the infrastructure keeps retrying on its own —
            usually resolves without changes), `ses_dkim_failed` /
            `ses_mail_from_failed` (a terminal verdict; re-verify after fixing),
            `ses_dkim_not_started` / `ses_mail_from_not_started` (the attribute
            was never configured on the identity — re-verify to push it), and
            `ses_not_verified_for_sending`. Absent when VERIFIED.
        feedback_enabled:
          $ref: '#/components/schemas/type_domains:FeedbackEnabled'
        subdomains_enabled:
          $ref: '#/components/schemas/type_domains:SubdomainsEnabled'
        tracking_enabled:
          $ref: '#/components/schemas/type_domains:TrackingEnabled'
        records:
          type: array
          items:
