...
Reference: https://docs.agentmail.to/api-reference/pods/threads/update
...
- `add_labels` (list of string, optional) — Labels to add to thread. Cannot be system labels.
- `remove_labels` (list of string, optional) — Labels to remove from thread. Cannot be system labels. Takes priority over `add_labels` (in the event of duplicate labels passed in).
...
HttpResponse<String> response = Unirest.patch("https://api.agentmail.to/v0/pods/pod_id/threads/thread_id")
...
$response = $client->request('PATCH', 'https://api.agentmail.to/v0/pods/pod_id/threads/thread_id', [
  'body' => '{}',
  'headers' => [
